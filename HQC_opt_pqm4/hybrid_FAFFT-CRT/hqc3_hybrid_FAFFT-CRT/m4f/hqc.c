...
void hqc_pke_keygen(uint8_t *ek_pke, uint8_t *dk_pke, uint8_t *seed) {
    ...
    CRT_fft_HQC3((uint32_t*)s, (uint32_t*)y, (uint32_t*)h);
...
void hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
    ...
    // Compute u = r1 + r2.h
    CRT_fft_HQC3((uint32_t*)(c_pke->u), (uint32_t*)r2, (uint32_t*)h);
    ...
    CRT_fft_HQC3((uint32_t*)tmp, (uint32_t*)r2, (uint32_t*)s);
...
uint8_t hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    ...
    // Compute u.y
    CRT_fft_HQC3((uint32_t*)tmp1, (const uint32_t*)y, (const uint32_t*)(c_pke->u));
    // Truncate(u.y)
    vect_truncate(tmp1);
