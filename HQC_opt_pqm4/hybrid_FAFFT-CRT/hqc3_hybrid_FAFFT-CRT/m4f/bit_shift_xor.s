.syntax unified
    .cpu cortex-m4
    .thumb
    .global bit_shift_xor_193_asm

@ -------------------------------------------------------------------------
@ void bit_shift_xor_193_asm(uint32_t* res, const uint32_t* A, 
@                            const uint32_t* idx, const uint32_t* cnt)
@ -------------------------------------------------------------------------
@ R0: res, R1: A(t), R2: Idx_Ptr, R3: Cnt_Ptr
@ Data: v1~v5 (R5~R9), Carry: v0 (R11)
@ -------------------------------------------------------------------------

bit_shift_xor_193_asm:
    PUSH    {R4-R11, LR}        @ Context Save

    @ 1. Loop Limit Setup
    @ Total 193 words. Process 5 words per block.
    @ Limit = 193 - 3 (tail) = 190 words.
    @ 190 words * 4 bytes = 760 bytes.
    MOVW    R12, #760           
    ADD     LR, R1, R12
    MOV     R11, #0
...
Next_Shift_Step:
    ...
    @ ---------------------------------------------------------
    @ 2. Tail Processing (3 Words left: t[190], t[191], t[192])
    @ ---------------------------------------------------------
Outer_Loop_End:
    @ R11 has Carry (t[189])
    LDMIA   R1, {R5-R7}         @ v1=t[190], v2=t[191], v3=t[192]
...
Tail_S0:
    ...
    LDMIA   LR, {R1, R10, R12}  @ Load 3 words
    EOR     R1,  R1,  R5
    EOR     R10, R10, R6
    EOR     R12, R12, R7
    STMIA   LR, {R1, R10, R12}
...
Tail_S_Inner:
    ...
    LDMIA   LR, {R9, R10, R12}  @ Load 3 words
    EOR     R9,  R9,  R5
    EOR     R10, R10, R6
    EOR     R12, R12, R7
    STMIA   LR, {R9, R10, R12}
