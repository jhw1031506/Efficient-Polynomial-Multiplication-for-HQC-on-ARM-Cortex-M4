...
#define gfmul_27 karat2_27_my
static inline void karat2_27_my(uint32_t *Out,  const uint32_t *A,  const uint32_t *B){
    static uint32_t D0[28], D1[28], D2[26], SAA[14], SBB[14];
    gfmul_14(D0, A, B);
...
#define gfmul_29 karat2_29_my
static inline void karat2_29_my(uint32_t *Out,  const uint32_t *A,  const uint32_t *B){
    static uint32_t D0[30], D1[30], D2[28], SAA[15], SBB[15];
    gfmul_15(D0, A, B);
    gfmul_14(D2, (A+15), (B+15));
...
#define gfmul_81 tc3_81_my
static inline void tc3_81_my(uint32_t *Out,  const uint32_t *A,  const uint32_t *B){
    const uint32_t *U0, *U1, *U2, *V0, *V1, *V2;
    static uint32_t W0[54], W1[57], W2[58], W3[58], W4[54], tmp[58];
    U0 = &A[0];
    U1 = &A[27];
    U2 = &A[54];
    V0 = &B[0];
    ...
    divide_by_x_plus_one(tmp, W2, 58);
    for (int32_t i = 0 ; i < 53 ; i++) {
        int32_t i1 = i + 1;
        tmp[i] = W3[i1] ^ W1[i1];
    }
    ...
    tmp[55] = W3[56];
    tmp[56] = W3[57];
    divide_by_x_plus_one(tmp, W3, 57);
    for (int32_t i = 0 ; i < 54 ; i++) {
        W1[i] ^= W2[i] ^ W4[i];
    }
    for (int32_t i = 54 ; i < 57 ; i++) {
...
void radix16_81(uint32_t *o, const uint32_t *a, const uint32_t *b)
{
	uint32_t sa[81], sb[81];
	poly_swap(sa, a, 81);
    poly_swap(sb, b, 81);
    gfmul_81(o, sa, sb);
    poly_invswap(o, 2*81);
